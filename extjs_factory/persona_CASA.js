	...
	*/
	function LoadAndShowGrid(recordArray, options, success){
		...
		grid.getSelectionModel().on('selectionchange', function(sm){
			//form.btnGuardar.setDisabled(sm.getCount() < 1);
			//form.btnEliminar.setDisabled(sm.getCount() < 1);
		});
...
Habilitar = function()
{
	...
	Ext.getCmp('ci_estudiante').focus(true);
}
...
Deshabilitar = function(ds)
{
	...
	var countRowsGrid = ds.getCount();    //Cuenta la cantidad de registros en el grid
	// verificamos la cantidad de registros
	if (countRowsGrid>0)
	{	
		// forzamos la seleccion del primer registro				
